...
`install.sh` запускает host bootstrap, создаёт `.env` из `.env.example`, автоматически генерирует все недостающие секреты, REALITY keys, выполняет деплой и сохраняет стартовые admin-данные в `/root/.xray-vpn-control-plane-install.txt`.
