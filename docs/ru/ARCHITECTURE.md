...
- `xray`
  - публичная точка входа VPN на `443/tcp`
...
- `caddy`
  ...
  - завершает TLS панели на `8443` в текущем single-node профиле
- host-level сервисы
  - firewall
  - Fail2ban
...
- UI restore ещё не завершён
- analytics и logs UX пока на MVP-уровне
- домен панели и публичный TLS-сертификат не настраиваются автоматически
