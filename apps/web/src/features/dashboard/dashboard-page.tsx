  ...
  const text =
    locale === 'en'
      ? {
          ...
          analyticsTitle: 'Analytics moved to a separate tab',
          analyticsItems: [
            'Daily traffic charts now live in Analytics instead of the operations dashboard.',
            'Per-client traffic rankings and live status are grouped in one dedicated screen.',
            'Use Dashboard for health and operations, and Analytics for traffic visibility.',
          ],
        }
      : {
          ...
          analyticsTitle: 'Аналитика вынесена в отдельную вкладку',
          analyticsItems: [
            'Дневные графики трафика теперь живут в разделе Аналитика, а не в операционном дашборде.',
            'Рейтинг клиентов по трафику и их live-статусы собраны в одном отдельном экране.',
            'Используй Дашборд для состояния системы и операций, а Аналитику для разбора нагрузки.',
          ],
        };
  ...
  return (
    <div className="page">
      ...
      <div className="content-grid">
        ...
        <SectionCard title={text.analyticsTitle}>
          <ul className="feature-list">
            {text.analyticsItems.map((item) => (
              <li key={item}>{item}</li>
            ))}
          </ul>
        </SectionCard>
