...
const XRAY_RUNTIME_CONFIG_PATH = '/var/lib/xray-vpn-control-plane/xray/config.json';
...
export class BackupsService implements OnModuleInit, OnModuleDestroy {
  ...
  private async buildArchive(snapshotId: string): Promise<BackupArchiveArtifact> {
    ...
    const archivePath = join(this.backupDir, `xray-vpn-control-plane-${timestamp}.tar.gz`);
