...
- optional `WireGuard` transport delivered from the same admin panel with per-client enable/disable controls
...
- `Docker Compose` deployment for `api`, `postgres`, `xray`, `caddy`, and optional `haproxy` SNI routing
- Scheduled local backups, host-side restore flow, import/export, public subscriptions, and TOTP 2FA for admin access
- dual endpoint variants for client delivery: preferred domain-based configs plus direct IP fallback where needed

### Quick Start

1. Clone the repository onto the target VPS.
2. Run `sudo bash install.sh` and enter only the server IP or domain when prompted.
3. Open the panel at the URL printed by the installer and sign in with the generated admin credentials.
...
- развёртывание через `Docker Compose` для `api`, `postgres`, `xray`, `caddy` и опционального `haproxy` SNI-router
...
2. Выполни `sudo bash install.sh` и укажи только IP или домен сервера.
3. Открой панель по адресу, который покажет installer, и войди с автоматически сгенерированными admin-данными.
