...
2. Run `sudo bash install.sh` for guided first install, bootstrap, secret generation, and deploy.
3. Open the panel on `https://YOUR_HOST:8443` and sign in with `INITIAL_ADMIN_*`.
4. Review [DEPLOY.md](./DEPLOY.md) if you prefer the manual path or want to customize the rollout.

### Common Install Examples

IP-only install with the VPS public address:

```bash
sudo git clone https://github.com/Twoia-Kotletochka/server-vpn.git /opt/server-vpn
cd /opt/server-vpn
sudo bash install.sh --host 203.0.113.10
```
...
sudo bash install.sh \
  ...
  --admin-password 'ChangeThisNow123!' \
  --non-interactive
```

For manual runs, omit `--admin-password` and let the installer prompt securely. Use `--non-interactive` only for automation or provisioning scripts.
...
2. Выполни `sudo bash install.sh`: скрипт подготовит хост, сгенерирует недостающие секреты и выполнит первый деплой.
3. Открой панель по адресу `https://ВАШ_ХОСТ:8443` и войди с `INITIAL_ADMIN_*`.
4. Если нужен ручной путь или более тонкая настройка, смотри [DEPLOY.md](./DEPLOY.md).

### Частые Примеры Запуска

Установка только по публичному IP VPS:
...
Для ручного запуска лучше не передавать `--admin-password` в командной строке, а дать `install.sh` запросить пароль интерактивно. `--non-interactive` нужен в первую очередь для automation и provisioning-скриптов.
