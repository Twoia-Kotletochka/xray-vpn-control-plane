...
INSTALL_SUMMARY_FILE="/root/.xray-vpn-control-plane-install.txt"
...
is_default_database_url() {
  local value="$1"
  [[ -z "${value}" || "${value}" == *"change-me@postgres:5432/xray_vpn_control_plane?schema=public"* ]]
}
...
postgres_user="${postgres_user:-xray_vpn_control_plane}"
postgres_db="${postgres_db:-xray_vpn_control_plane}"
