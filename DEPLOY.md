...
`install.sh` runs host bootstrap, creates `.env` from `.env.example`, generates every missing secret automatically, provisions REALITY keys, deploys the stack, and stores the initial admin credentials in `/root/.xray-vpn-control-plane-install.txt`.
