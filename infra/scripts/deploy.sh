...
normalize_legacy_env_paths() {
  ...
  case "${current_backup_dir}" in
    "${ROOT_DIR}/infra/backup/output"|"/opt/xray-vpn-control-plane/infra/backup/output")
      sed -i 's#^BACKUP_DIR=.*#BACKUP_DIR=/var/backups/xray-vpn-control-plane#' "${ENV_FILE}"
      echo "Normalized BACKUP_DIR to container path /var/backups/xray-vpn-control-plane"
      ;;
  esac

  case "${current_host_backup_dir}" in
    ""|"/var/backups/xray-vpn-control-plane"|"/absolute/path/to/project/infra/backup/output")
